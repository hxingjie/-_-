...
private:
	...
	void DrawCell(int col, int row, unsigned int color) const;
public:
	State(const char* stageData, int size);
	void GameLogitUpdate(char input);
	void Draw() const;
...
void mainLoop(){
	...
	//刷新
	gState->GameLogitUpdate(input);

	//绘制
	gState->Draw(); // Framework::update() 结束才会完成绘制，所以Draw写在最后面
...
void State::Draw() const {
	...
	for (int row = 0; row < mHeight; row++){
		for (int col = 0; col < mWidth; col++){
			...
			//vram[row * windowWidth + col] = color;
...
void State::DrawCell(int col, int row, unsigned int color) const {
	unsigned int* vram = GameLib::Framework::instance().videoMemory();
	int windowWidth = GameLib::Framework::instance().width();
	for (int i = 0; i < 16; i++) {
		for (int j = 0; j < 16; j++) {
			vram[(row*16+i) * windowWidth + (col*16+j)] = color;
		}
	}

	for (int i = 0; i < 50; i++) {
		for (int j = 112; j <= i+112; j++) {
			vram[i * windowWidth + j] = 0x000000ff;
		}
	}
	for (int i = 50; i < 100; i++) {
		for (int j = 112; j < 162+50-i; j++) {
			vram[i * windowWidth + j] = 0x000000ff;
		}
	}
}

void State::GameLogitUpdate(char input){
	//移动量变换
	int dCol = 0, dRow = 0;
	switch (input){
